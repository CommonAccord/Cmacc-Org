...
Date.sec=The date of this Term Sheet is {EffectiveDate.YMD}.  The "{_Effective_Date}" is the date on which the parties execute the definitive license agreement (the "{Def.License_Agreement.Target}") and close the financing round (the "Closing Date"), anticipated to be {ClosingDate.YMD}.

Date.=[G/Z/Base]

Licensor.Ti={Def.Licensor.Target} 	

Licensor.sec={Licensor.N,E,A}

Licensor.=[G/Z/Base]

Licensee.Ti={Def.Licensee.Target}

Licensee.sec={Company.US.N,E,A} and/or its {_Affiliate}(s). The term “{Def.Licensee.Target}” will only include {_Affiliate}(s) to the extent that the {_License} is assigned to such {_Affiliate} or an {_Affiliate} is granted a {_Sublicense}.

Licensee.=[G/Z/Base]

Territory.Ti=Territory

Territory.sec=“{Def.Territory.Target}” means worldwide.
...
PatentRights.Ti={Def.Patent_Rights.Target}	
...
DevelopmentMilestonePayments.2.sec=Submission of the first IND for a {_Licensed_Product}: [$____]  

DevelopmentMilestonePayments.3.sec=FDA approval of a {_Licensed_Product}: [$____]

DevelopmentMilestonePayments.4.sec={_First_Commercial_Sale} of a {_Licensed_Product}: [$____]
...
RoyaltyTerm.Ti=“{Def.Royalty_Term.Target}”
...
DiligenceMilestone.0.sec=In addition, {_Licensee} (either itself or through the acts of a {_Sublicensee}) is required to achieve the following due diligence milestones (the “{Def.Diligence_Milestone.Target}”) by the dates set forth below (the “{Def.Achievement_Date.Target}”):
...
PatentExpenseReimbursement.sec={_Licensee} will bear all out-of-pocket costs incurred by {_Licensor} for Patent Actions (“{Def.Patent_Costs.Target}”). {_Licensee} will reimburse {_Licensor} [within 30 days after Licensee’s receipt of an invoice from {_Licensor}] for {_Patent_Costs} incurred by {_Licensor} before the {_Effective_Date} and during the term of the {_License_Agreement}.  Alternatively, if {_Licensor} requests, {_Licensee} will pay ongoing {_Patent_Costs} in advance or under a reasonable direct billing arrangement with Licensor’s patent counsel.   {_Licensor} will provide {_Licensee} with documentation of the {_Patent_Costs}.
...
Def.=[G/NVCA/2020/LS-License-TermSheet/Z/Def/Target.md]
