Note=YCombinator SAFE Note - Base form (v2-0, 2026 edition).  Jurisdiction-neutral.  All US-isms, Canadianisms, etc. live in /Form/Z/ overlays.  Specific deal-types (Cap, Discount, MFN) extend this by overriding the placeholders in the {Event.Equity.*.ConversionTarget.cl}, {Intro.DealTerm.sec}, and {Def.sec} slots.

CodersNote=Relative to the 2020 Base: (i) "Post-Money Valuation Cap" and "Discount Rate" are now defined terms with their own Def entries, (ii) the Investor Rep omission of "a" before "valid and binding" is fixed, (iii) Oxford commas are normalised, (iv) the Unissued_Option_Pool is hyperlinked like every other defined term, (v) the Intro's per-form deal-term line is factored into a single {Intro.DealTerm.sec} slot, (vi) the US-specific legend/tax/notice/law text has moved to the US overlay so the Base reads as pure structure, and (vii) cross-references use named anchors throughout (no brittle numeric anchors).

Ti=<center>{Note.Title}</center>

Note.Title=Simple Agreement for Future Equity - Base (v2-0)

Intro.1.sec={Legend.sec}

Legend.sec={Legend.Home.sec}
...
Note=The {Intro.DealTerm.sec} slot is filled by the Cap, Discount, or MFN form.  MFN leaves it blank.

Intro.8.sec=See Section {Def.Xnum} for certain additional defined terms.

Intro.=[G/Z/paras/s8]

Intro.sec={Intro.1.sec}{Intro.2.sec}{Intro.3.sec}{Intro.4.sec}<br>{Intro.5.sec}<br><br>{Intro.6.sec} {Intro.DealTerm.sec}<br><br>{Intro.8.sec}<br>
...
Note=Definitions are listed alphabetically.  Each is a {Def.<Term>.sec} slot that a variant form may override, include, or omit.  The {Def.sec} slot assembles the final list; variants may swap the list without touching individual definitions.
...
Def.Discount_Rate.sec="{DefT.Discount_Rate}" has the meaning (where applicable) set forth above in the introductory paragraphs of {_this_Safe}.

Note=The Base defines {_Discount_Rate} so the MFN and Cap-only variants can reference it without error when included through pro rata side letters, boilerplate, or cross-instrument MFN comparisons.  Forms that do not use a Discount Rate omit {Def.Discount_Rate.sec} from their {Def.sec} list.
...
Def.Post-Money_Valuation_Cap.sec="{DefT.Post-Money_Valuation_Cap}" has the meaning (where applicable) set forth above in the introductory paragraphs of {_this_Safe}.

Note=The Base defines {_Post-Money_Valuation_Cap} for the same reason as Discount Rate: MFN compares terms across convertible securities, and having the term hyperlinked to a canonical definition keeps cross-document references resolvable.
...
Def.Subsequent_Convertible_Securities.sec="{DefT.Subsequent_Convertible_Securities}" means convertible securities that the {_Company} may issue after the issuance of this instrument with the principal purpose of raising capital, including, but not limited to, other {_Safes}, convertible debt instruments, and other convertible securities.  {_Subsequent_Convertible_Securities} excludes: (i) options issued pursuant to any equity incentive or similar plan of the {_Company}; (ii) convertible securities issued or issuable to (A) banks, equipment lessors, financial institutions, or other persons engaged in the business of making loans pursuant to a debt financing or commercial leasing, or (B) suppliers or third-party service providers in connection with the provision of goods or services pursuant to transactions; and (iii) convertible securities issued or issuable in connection with sponsored research, collaboration, technology license, development, OEM, marketing, or other similar agreements or strategic partnerships.
...
DefT.=[G/YCombinator-SAFE/2026/Form/Z/Def-Target.md]

_=[G/YCombinator-SAFE/2026/Form/Z/Def-Link.md]


_This_Safe=<a href='#DefT.this_Safe' class='definedterm'>This Safe</a>

_this_Safe=<a href='#DefT.this_Safe' class='definedterm'>this Safe</a>

_Unissued_Option_Pool=<a href='#DefT.Unissued_Option_Pool' class='definedterm'>Unissued Option Pool</a>

_Discount_Rate=<a href='#DefT.Discount_Rate' class='definedterm'>Discount Rate</a>

_MFN_Notice=<a href='#DefT.MFN_Notice' class='definedterm'>MFN Notice</a>


CodersNote=Cross-references are by anchor name rather than by position so they remain correct when sections are added or removed.
...
Rep.Company.AllConsents.Xnum={Rep.Company.Xnum}.<a href='#Rep.Company.AllConsents.sec'>d</a>


Note=Summary Row - deal-points snapshot for dashboards / side-by-side ledgers.
...
Note=Defaults for the deal-points row when a field is not applicable to a given form.
