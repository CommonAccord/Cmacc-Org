...
Designate.Sec=<b>DESIGNATING {PROTECTED MATERIAL}</b><br><ol><li>{Designate.WithRestraint.Sec}</li><li>{Designate.Manner.Sec}</li><li>{Designate.Failure.Sec}</li></ol>

Challenge.Sec=<b>CHALLENGING CONFIDENTIALITY DESIGNATIONS</b><br><ol><li>{Challenge.Timing.Sec}</li><li>{Challenge.Confer.Sec}</li><li>{Challenge.Judge.Sec}</li></ol>

Access.Sec=<b>ACCESS TO AND USE OF {PROTECTED MATERIAL}</b><br><ol><li>{Access.Principle.Sec}</li><li>{Access.Confidential.Who.Sec}</li><li>{Access.HighlyConfidential.Who.Sec}</li><li>{Access.HighlyConfidential.Procedure.Sec}</li></ol>

SourceCode.Sec=<b>SOURCE CODE</b><br> <ol><li>{SourceCode.Designate.Sec}</li><li>{SourceCode.Protection.Sec}</li><li>{SourceCode.Access.Sec}</li><li>{SourceCode.PaperCopy.Sec}</li><li>{SourceCode.Care.Sec}</li></ol>

OtherLit.Sec=<b>{PROTECTED MATERIAL} SUBPOENAED OR ORDERED PRODUCED IN OTHER LITIGATION</b><br>{OtherLit.Intro}:<ol><li>{OtherLit.NotifyDesignatingParty.Sec};</li><li>{OtherLit.NotifyOtherLitigant.Sec}; and</li><li>{OtherLit.Cooperate.Sec}.</li></ol>{OtherLit.Wait.Sec}

Non-PartyInfo.Sec=<b>A {NON-PARTY}'S {PROTECTED MATERIAL} SOUGHT TO BE PRODUCED IN THIS LITIGATION</b><br><ol><li>{Non-PartyInfo.IsSubject.Sec}</li><li>{Non-PartyInfo.Procedure.Sec}</li><li>{Non-PartyInfo.TimeForObjection.Sec}</li></ol>

Misc.Sec=<b>MISCELLANEOUS</b><br><ol><li>{Misc.FurtherRelief.Sec}</li><li>{Misc.OtherObjection.Sec}</li><li>{Misc.ExportControl.Sec}</li><li>{Misc.NoOpenFiling.Sec}</li></ol>
...
Access.HighlyConfidential.Procedure.Sec=<b>Procedures for Approving or Objecting to Disclosure of {C.2or3} Information or Items to {Designated House Counsel} or {Expert}s</b><br><ol><li>{Access.Procedure.Request.Sec}</li><li>{Access.Procedure.TimeForObjection.Sec}</li><li>{Access.Procedure.ConferOnObjection.Sec}</li></ol>
...
Misc.FurtherRelief.Sec=<b>Right to Further Relief</b><br> Nothing in this Order abridges the right of any person to seek its modification by the court in the future.
