...
A record `prefix.=[path/filename]` causes the namespace of the file at `path/filename` to be folded into the current namespace.

### 5.1 Prefixed Inclusion

Given a record whose key is `P` (typically ending in a dot, e.g. `Investor.`) and whose value is `[path/filename]`, every binding `k -> v` in the included namespace `P'` is contributed to `N` as:
...
Note that a record `Company.=[G/U/Who/acme_incorporated.md]` not only includes the file's bindings under the `Company.` prefix, but also (by §4.1) attempts to bind the bare key `Company.` itself as `[G/U/Who/acme_incorporated.md]`. That binding is harmless if no placeholder directly references `{Company.}`.

### 5.2 Unprefixed Inclusion

If the key in an inclusion record is the empty string — i.e., the record reads `=[path/filename]` — the included namespace is merged with **no prefix**. Bindings flow in under their own names. This is how a "form" or "template" file is pulled into an "instance" file.
...
-MUST be lazy: it should not read or build the namespace of any file (directly or recursively) until rendering reveals it as a possible source for an actually demanded `{Placeholder.Key.Name}`.
...
- MUST report missing target files as a recoverable error: the inclusion contributes nothing to `N`, but rendering of the rest of the file proceeds. (The current implementation merely dies without any error message, a known deficiency.)
...
A placeholder whose name is not bound in `N` is emitted **verbatim**, braces and all. This is intentional. It is a survivable failure mode that doubles as a debugging aid: the rendered output shows exactly which keys were missing. For convenience, the current implementation wraps the `{` and `}` in a `<span class="missing">` to make them visually distinctive (red).
...
- impose a finite maximum expansion depth and fail/truncate when exceeded.  (The current implementation has no protection against a circular reference or self-reference.)
...
Authors conventionally define a key called `r00t` as the "whole document" rendering target, and keys like `Sec` or `Doc` as alternative views. The renderer attaches no special meaning to these names; they are simply the keys most often passed as the second argument to `render`.
...
- **Why first-writer-wins?** It lets concrete instance files override defaults in a generic form, as well as state the number of facts that the generic form intentionally leaves to the user. This leaves the user free to make whatever changes seem appropriate.  The instance is a thin diff against a shared template as well as a listing of deal points.
...
- **Why filesystem paths?** The store is an ordinary directory of ordinary text files. Git, diff, grep, code review, and IDEs all work without adaptation.  This is particularly useful for hosting on GitHub, which is file and path oriented. A database or key/value store would make sense for heavy-duty production use, with the git/path/files approach being appropriate for legal text collaboration (and codification) on the model of source code collaboration.
